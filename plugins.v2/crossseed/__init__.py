...
class CrossSeedHelper(object):
    ...
    @staticmethod
    def get_local_torrent_info(torrent_path: Union[Path, str]) -> Tuple[Optional[TorInfo], str]:
        try:
    ...
    def get_target_torrent(
        ...
        try:
            ...
            if isinstance(rsp_body.get("data"), dict):
...
class CrossSeed(_PluginBase):
    ...
    def init_plugin(self, config: dict = None):
        ...
        if config:
            ...
            self._torrentpaths = [p.strip() for p in self._torrentpath.strip().split(",") if p.strip()]
            ...
            for site_key in self._token.strip().split("\n"):
                if not site_key.strip():
                    continue
                # 【修复核心BUG】：将 line 改为循环变量 site_key
                site_key_arr = [s.strip() for s in re.split(r"[:：]", site_key) if s.strip()]
                if not site_key_arr:
                    continue
    ...
    def auto_seed(self):
        ...
        for idx, service in enumerate(self.service_infos.values()):
            ...
            # 获取对应的种子目录路径
            torrent_dir = self._torrentpaths[idx] if idx < len(self._torrentpaths) else None
            if not torrent_dir:
                logger.error(f"下载器 {downloader} 未设置对应的种子保存路径，跳过扫描")
                continue
            ...
            for torrent in torrents:
                ...
                # 获取种子文件路径
                torrent_path = Path(torrent_dir) / f"{hash_str}.torrent"
                torrent_info = None
                ...
                for tracker in tracker_urls:
                    for site_config in self._site_cs_infos:
                        if site_config.passkey and site_config.passkey in tracker:
                ...
                if self._nopaths and save_path:
                    ...
                    for nopath in self._nopaths.split('\n'):
                        if nopath.strip() and os.path.normpath(save_path).startswith(os.path.normpath(nopath.strip())):
                ...
                if torrent_labels and self._nolabels:
                    ...
                    for label in self._nolabels.split(','):
                        if label.strip() and label.strip() in torrent_labels:
        ...
        self.__update_config()
        if self._notify and (self.success or self.fail):
            self.post_message(
        ...
        for site_config in self._site_cs_infos:
            ...
            for i in range(0, len(pieces_hashes), chunk_size):
                ...
                if not chunk_tors and err_msg:
                    ...
                    logger.info(
                        f"站点{site_config.name}本批次的可辅种/查询数={len(chunk_tors or [])}/{len(chunk)},进度={i + 1}/{total_size}"
                    )
                    if chunk_tors:
                        remote_tors.extend(chunk_tors)
            ...
            for tor_info in not_local_tors:
                if self._event.is_set():
                    logger.info("辅种服务停止")
                    return
                if not tor_info or not tor_info.torrent_id or not tor_info.pieces_hash:
                    continue
    ...
    def __add_recheck_torrents(self, service: ServiceInfo, download_id: str):
        logger.info(f"添加校验检查任务：{download_id} ...")
        if service.name not in self._recheck_torrents:
